...
#include <Core/Math/AgnosticDataTypes.h>
...
namespace Primitives3D
{
    ...
    inline vec3f MakeUVSperePos(float radius, float phi, float theta)
    {
        const float s = mutil::sinf(phi);

        // spherical to cartesian coordinate conversion
        return vec3f{ radius * s * mutil::cosf(theta), radius * mutil::cosf(phi), radius * s * mutil::sinf(theta) };
    }

    inline vec3f MakeUVSphereSmoothNormal(const vec3f& p) { return p.normalized(); }

    inline vec3f MakeFlatNormal(const vec3f& p0, const vec3f& p1, const vec3f& p2)
    {
        return (p1 - p0).cross(p2 - p0).normalized();
    }

    inline void MakeUVSphereNormals(const vec3f pos[], vec3f norm[], bool smoothed, unsigned count = 4)
    {
        vec3f normal = MakeFlatNormal(pos[0], pos[1], pos[2]);
    ...
    MakeSphereUVs(float phiStep, float thetaStep, unsigned sliceCount, unsigned i, unsigned j, bool rightEdge)
    {
        ...
        if (rightEdge && j == sliceCount)
            return vec2f{ 1.0f, phi / mutil::PI };

        float u = theta / mutil::TWO_PI; // [0,1)
        float v = phi / mutil::PI;       // [0,1]
        return vec2f{ u, v };
    }
        ...
        bool                   rightHanded = true)
    {
        ...
        const float phiStep   = mutil::PI / static_cast<float>(stackCount);
        const float thetaStep = mutil::TWO_PI / static_cast<float>(sliceCount);
        ...
        bool                   flip_UVs    = false)
    {
        ...
        const vec3f norms[4] = { normal, normal, normal, normal };
        // uvs
        vec2f uvsArr[4]      = {
            { 0.0f, 0.0f }, // 0 TL
            { 0.0f, 1.0f }, // 1 BL
        ...
        bool                   flip_UVs    = false)
    {
        ...
        {
            float z = halfDepth - static_cast<float>(i) * dz; // current z position
            for (unsigned int j = 0; j <= gridCountX; ++j)
            {
                float x = -halfWidth + static_cast<float>(j) * dx; // current x position
                // position
                ...
                if SAME (VertexType, PU, PUN)
                {
                    float u = static_cast<float>(j) * du;
                    float v = static_cast<float>(i) * dv;
                    if (flip_UVs)
                        v = 1.0f - v;
